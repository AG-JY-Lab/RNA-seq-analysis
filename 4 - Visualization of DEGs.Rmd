...
DEG Bar graph
```{r echo=FALSE, warning=FALSE, message=FALSE}
DEGs_up <- RNA_seq_sigDEGS %>% filter(log2FoldChange > 0)
DEGs_down <- RNA_seq_sigDEGS %>% filter(log2FoldChange < 0)
...
res_table_rn <- RNA_seq_res_table_tb %>% column_to_rownames(var = "Ensembl ID")
genes <- rownames(res_table_rn)
volcano_plot_data <- data.frame(genes,
                                log2FC = res_table_rn[,2] ,
                                logqval = -log10(res_table_rn[,5]))
...
normalized_DEGs_df <- RNA_seq_normalized_counts_df[,c(1:9)] %>% 
  filter(`Ensembl ID` %in% RNA_seq_sigDEGS$`Ensembl ID`) %>% 
  data.frame() %>%
  column_to_rownames(var = "Ensembl.ID")

# Run ComplexHeatmap
normalized_DEGs_matrix <- as.matrix(normalized_DEGs_df[, grep("_", colnames(normalized_DEGs_df))])
base_mean = rowMeans(normalized_DEGs_matrix)
matrix_scaled = t(apply(normalized_DEGs_matrix, 1, scale))

sample_names <- colnames(normalized_DEGs_matrix)

# Match the sample names with their corresponding sample type from the metadata
heatmap_annotation <- HeatmapAnnotation(Sample = sample_names,
                                        Condition = RNA_seq_metadata$sampletype[
                                          match(sample_names, rownames(RNA_seq_metadata))
                                          ],
                                        annotation_name_side = "left")
set.seed(123)
heatmap_list <- Heatmap(matrix_scaled,
